...
//caigz add 主频计算公式  
//PLL_VCO = (HSE_VALUE or HSI_VALUE / PLL_M) * PLL_N  外部晶振HSE_VALUE=12M;  PLL_M=12; PLL_N=336,得出PLL_VCO=336。
//SYSCLK = PLL_VCO / PLL_P  							SYSCLK=336 / 2。
...
void SystemInit(void)
{
  ...
  /* Set HSION bit 使能HSI内部振荡器 */ 
  RCC->CR |= (uint32_t)0x00000001;

  /* Reset CFGR register 清零RCC->CFGR寄存器*/
  RCC->CFGR = 0x00000000;

  /* Reset HSEON, CSSON and PLLON bits 将HSE设置为OFF,CSSON和PLL设置为OFF*/
  RCC->CR &= (uint32_t)0xFEF6FFFF;

  /* Reset PLLCFGR register 复位RCC->PLLCFGR寄存器，高四位保留，将PLLQ=4，将PLL和PLLIS时钟源设置为HSI,*/
  RCC->PLLCFGR = 0x24003010;

  /* Reset HSEBYP bit 将HSEBYP位清零，即HSE时钟不被旁路了，使用无源时钟芯片*/
  RCC->CR &= (uint32_t)0xFFFBFFFF;

  /* Disable all interrupts 				Disable 所有中断*/
  RCC->CIR = 0x00000000;
...
/******************************************************************************/
  ...
  /* Enable HSE  将RCC->CR寄存器的第16位置1，使能Enable HSE*/
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    /* HCLK = SYSCLK / 1	HCLK 不分频 */
    RCC->CFGR |= RCC_CFGR_HPRE_DIV1;

#if defined (STM32F40_41xxx) || defined (STM32F427_437xx) || defined (STM32F429_439xx)      
    /* PCLK2 = HCLK / 2 PCLK2  二分频 */
    RCC->CFGR |= RCC_CFGR_PPRE2_DIV2;
    
    /* PCLK1 = HCLK / 4		PCLK1 四分频 */
