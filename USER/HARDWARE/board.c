...
void bsp_Board_Init(void)
{
	...
	bsp_Led_Init();  								//板载显示灯初始化
	AM2320_and_iic_init();					//温湿度传感器模块初始化
	bsp_Limit_SW_Init();						//红外、巢穴开关初始化
	bsp_enccoder_Init();						//从动轮轮子编码器
	bsp_Video_Trans_PWR_Init();			//摄像头电源初始化
	
	bsp_ADC_Init();									//ADC采集初始化
	bsp_slave_cal_speed_Init();			//从电机模拟速度计算初始化
	bsp_master_cal_speed_Init();		//主电机模拟速度计算初始化
	bsp_Pressure_Sensor_Init();			//压力传感器ADC采集初始化
...
//	u32 Datalen;
	...
	gGPS_Value = GPS_Info.currentLatitude;
