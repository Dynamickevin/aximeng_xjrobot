...
CmdParaOneItem* pitem;

#define CMD_PARA_IS_CHAR_DIVIER(cur_ch)  (('='==(cur_ch)) || (' '==(cur_ch)) || ('\t'==(cur_ch)))
    ...
    )
{
    ...
    *str_cmd++ = '=' ;
    str_cmd = int_to_str( val , str_cmd );
    *str_cmd++ = ' ';//'\t' ;
    *str_cmd = ' ';//0;
    return str_cmd;
}
    ...
    )
{
    ...
    *str_cmd++ = '=' ;
    ...
    else
    {
        while(val_cnt)
        {
            str_cmd = int_to_str( *vals++ , str_cmd );
            *str_cmd++ = ',' ;  //逗号作为分隔符
            val_cnt--;
        }
        str_cmd--; //删除最右一个,
...
/************************************************* 
*Return:      char* 返回结束位置
*DESCRIPTION: 生成参数字符串，参数值为 多个long数据 name=234,255\t
*************************************************/
char* cmd_para_build_para_string_longs(
    __OUT_ char*  str_cmd ,
    __IN__ const char*  str_name ,
    __IN__ long*    vals ,
    __IN__ int     val_cnt
