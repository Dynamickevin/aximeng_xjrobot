...
typedef struct
{
    ...
    u16   real_slv_up_press_to_adg;  //上升过程中，目标压力值；如果当前是处于未压紧，打滑状态，目标压力值增加些
    u16   test_out_debug;
...
typedef struct{
    ...
    bool bOnBridge_needmove; //在桥上，需要运动
}BatAutoCtrl;
extern BatAutoCtrl gBatAutoCtrl;
#define BAT_CHARGE_OPEN()    GpioSetBit(GPIO_BAT_CHARGE_EN,0)
#define BAT_CHARGE_CLOSE()   GpioSetBit(GPIO_BAT_CHARGE_EN,1)
/************************************************* 
*Return:	void
