...
{
    char* str = (char*)(g_zt_msg.sendbuf);   //保存指针首地址
...
int zt_build_send_state_string(u32 stateFlag,uint8 id,uint8 kind)
{
	...
	{
        str = cmd_para_build_para_string_int( str , "press" , (int)(gPressFilter.val) );
    }
    if( stateFlag & BUILD_STATE_FLAG_AM2320 )
	{
        str = cmd_para_build_para_string_float( str , "temp" , AM2320_get_temper()*0.1f , 1 );
        //str = cmd_para_build_para_string_float( str , "temp" ,250*0.1f, 1 );
        str--; 
		//*str++ = '^';  *str++ = 'C';  
		*str++ = ' ';//'\t';添加一个字符 空格
        str = cmd_para_build_para_string_float( str , "humi" , AM2320_get_humidi()*0.1f , 1 );
...
void Hello(uint8 com,uint8* cmd)   			 //6
{
    ...
    if( com == ID_LINUX )
	{
       ...
       if ( vid >= 0 ) {
        ...
        gCmdParaVal_Ints[0] = 2400;
        gCmdParaVal_Ints[1] = 5000;
    ...
    else
    {
		...
		else
		{
        	...
        	//if ( vid > 0 ) { gUserAllCfg.SlvMtCfg.press_ok_min = atoi( gCmdParaInfos.items[vid].value ); }
       		//if ( vid > 0 ) 
			gUserAllCfg.SlvMtCfg.press_ok_min = (u16)((gCmdParaVal_Ints[0])); //单位为牛顿
        	//vid = cmd_para_get_id_by_name(&gCmdParaInfos,"pmax");
       		...
       		//if ( vid > 0 ) 
       		gUserAllCfg.SlvMtCfg.press_ok_max = (u16)((gCmdParaVal_Ints[1])); //单位为牛顿
        	...
        	//if ( vid > 0 )
        	gUserAllCfg.SlvMtCfg.press_mst_stop_min = (u16)((gCmdParaVal_Ints[2])); //单位为牛顿
        	...
        	//if ( vid > 0 ) 
         	gUserAllCfg.SlvMtCfg.press_mst_stop_max = (u16)((gCmdParaVal_Ints[3])); //单位为牛顿
    	}
...
//AT+PtzSet=90,-20,0,10 方位角、俯仰角、横滚角、焦距
	...
	if ( box_str_cmp(gCmdParaInfos.items[0].name , "StateSw" ) == 0 )
     {
		...
		else if( box_str_cmp(gCmdParaInfos.items[0].value , "ChgBk" ) == 0 )
		 {
            gRbtState.StateSwitchFlag[0] =  'C' ;
    ...
    if ( temp!=NOT )
	{
		switch ( temp )
		{
			...
			case 2:		
				...
				break;
			case 3:    
				set_SystemReset();		//系统复位
				break;
			case 4:
				SetSlaveMotor(com,dp);	//从电机控制指令
				break;
			case 5:
				SetMasterMotor(com,dp);	//主电机控制指令
				break;
			case 6:
				Hello(com,dp);			//与Linux交互
				break;
			case 7:
				GetState(com,dp);		//获取机器人自身状态
				break;
			case 8:
				SlvCFG1(com,dp);		//修改压力值参数 AT+SlvCFG1=8 pmin=70 pmax=90 StopMin=20 StopMax=125 
				break;	
			case 9:
				GetSlvCFG(com);			//获取	从动轮		配置参数函数
				break;	
			case 10:
				BatCFG(com,dp);			//修改电池充电配置参数	AT+BatCFG=0 BatNoMove=21.2 ChgNeed=22.2 ChgFull=25.2 CRC=2982
				break;	
			case 11:
				Lcmd(com,dp);			//
				break;	
			case 12:
				OpenDev(com,dp);		//打开设备电源
				break;
			case 13:
				CloseDev(com,dp);		//关闭设备电源
				break;	
			case 14:
				LinuxRst(com);			//
				break;
			case 15:
				Rf433Cfg(com,dp);		//配置RF433模式
				//GpioSetL(GPIO_LED_SHOW1);
				break;	
			case 16:
				Rf433Set(com,dp);		//修改配置RF433参数
